...
Описание:
  Модуль позволяет автоматически выгружать архивные данные на сайт rp5.ru, используя API (https://rp5.ru/docs/sgate/ru). Модуль совместим с Python 3 (обновлен с предыдущих версий для Python 2). Теперь модуль также передает данные об атмосферном давлении и количестве осадков (если они доступны в вашей системе WeeWX).
...
Автоматическая установка с помощью wee_extension:
  1. Загрузите последнюю версию Модуля.
      # Перейдите на страницу релизов https://github.com/sapegin-o1eg/weewx-rp5/releases и скопируйте ссылку на архив последней версии.
      # Пример команды для скачивания:
      # wget -P /tmp https://github.com/sapegin-o1eg/weewx-rp5/releases/download/vX.Y.Z/weewx-rp5-X.Y.Z.tar.gz
  ...
  3. Установите модуль используя утилиту wee_extension
      cd /home/weewx/bin
      ./wee_extension --install=/tmp/weewx-rp5-X.Y.Z.tar.gz # Замените X.Y.Z на актуальную версию
...
Ручная установка:
  ...
  3. Распакуйте установочный файл
      tar xvfz /tmp/weewx-rp5-X.Y.Z.tar.gz # Замените X.Y.Z на актуальную версию

  4. Скопируйте распакованные файлы
      # Предполагается, что архив распаковывается в директорию с именем weewx-rp5-X.Y.Z
      cp weewx-rp5-X.Y.Z/bin/user/*.py /home/weewx/bin/user # Замените X.Y.Z на актуальную версию
